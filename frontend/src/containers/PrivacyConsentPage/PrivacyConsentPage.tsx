      ...
      <div className='content_privacy-body'>
        ...
        {data.main_items.map((item: {
          ...
          if (item.specific_to === "" || item.specific_to === "scholaris") {
            return (
              <>
                <h3 key={`h3-${index}`} className='content_privacy-body-subheader'>{item.heading}</h3>
                <p key={`p-${index}`} className='content_privacy-body-text'>{item.content}</p>
                {item.sub_contents.length > 0 ? item.sub_contents.map((sub_content: {heading: string, content: string}, ind: number) => {
                  return (
                    <>
                      <h4 key={`h4-${ind}`} className='content_privacy-third-level-heading'>{sub_content.heading}</h4>
                      <p key={`p-${ind}`} className='content_privacy-body-text'>{sub_content.content}</p>
                    </>
                  )
                }) : null}
              </>
