class Scholarship < ApplicationRecord
  ...
  scope :filtered, ->(params) {
    ...
    results = results.where("DATE(start_date) >= ?", Date.parse(params[:start_date])) if params[:start_date].present?
    results = results.where("DATE(due_date) <= ?", Date.parse(params[:due_date])) if params[:due_date].present?
